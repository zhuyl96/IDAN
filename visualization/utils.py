...
def w2v_prep(data_path, stopwords_path, lex_basepath, lexicon_names, w2v_path, seq_len, senti_seq_len):
    """
    ...
    train_data['review_w2v'] = train_data['review'].map(
        lambda x: [word for word in jieba.cut(str(x), cut_all=False) if word not in stopwords])
    ...
    # 前一半是句子 后一半是情感信息
    train = pad_sequences(temp[:int(len(temp) / 2)], maxlen=seq_len, padding='post')
...
def w2v_prep_(data_path, stopwords_path, w2v_path, seq_len):
    """
    ...
    # 以train_data中的词作为word_dict
    word_dict = {}
    word_dict.update({'PAD': 0, 'UNK': 1})  # 没有使用这两个 不过也留着
    n = 2
